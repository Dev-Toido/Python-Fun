...
def intro():
    ...
    print("{:^80s}".format("MODIFIED BY: Vivek Garai"))    
    print("=" * 80)
    print()
    time.sleep(2)
def create_record():
    ...
    if record is None:
        ...
        print('Duplicate Data found!!! Please check once more!!')
    time.sleep(2)
def search(name):
    sql = "SELECT * FROM book WHERE name = %s"
    value = (name,)
...
def modify_record(name):
    ...
    if record is None:
        ...
        while True:
            ...
            print("5. BACK")
            print()
            ch = int(input("Select Your Option (1-5): "))
            if ch == 1:
                ...
                sql = "UPDATE book SET email = %s WHERE name = %s"
